...
ShallowWaterScene* ShallowWaterScene::p_myself = nullptr;
Screen* ShallowWaterScene::p_screen = nullptr;
const float ShallowWaterScene::pi = 3.1415926535897932384626433832795f;
const vec3 ShallowWaterScene::v3SunLight{ 1.2f, 1.2f, 0.9f };
const vec3 ShallowWaterScene::v3MoonLight{ 0.1f, 0.2f, 0.7f };


ShallowWaterScene* ShallowWaterScene::initializeScene(uint32_t init_screen_width, uint32_t init_screen_height,
    const std::string& topography_file_name, const std::string& shader_base_catalog, const std::string& texture_base_catalog, uint32_t reflection_map_resolution /* = 1024U */)
{
    if (!p_myself)
    {
        ...
        p_screen->setStringName("Shallow Water Technical Demo (created by Alex Bibov aka lexer42152)");
        ...
        //Create the scene
        p_myself = new ShallowWaterScene(topography_file_name, reflection_map_resolution);
...
void ShallowWaterScene::destroyScene()
{
    if (p_myself)
    {
        delete p_myself;
...
bool ShallowWaterScene::updateScene()
{
...
void ShallowWaterScene::onScreenRedraw(Screen& target_screen)
{
    if (p_myself)
    {
...
void ShallowWaterScene::onSceneRedraw(Framebuffer& target_framebuffer)
{
    ...
    target_framebuffer.readPixelsIntoTexture(tiny_world::RenderingColorBuffer::COLOR0, p_myself->refraction_map, 0, 0, 0, 0, 0, 0, screen_size.first, screen_size.second);
    p_myself->water.updateRefractionTexture(p_myself->refraction_map, p_myself->linear_depth_buffer);
    p_myself->water.applyViewProjectionTransform(p_myself->main_camera);
    for (uint32_t i = 0; i < p_myself->water.getNumberOfRenderingPasses(TW_RENDERING_MODE_DEFAULT); ++i)
    {
        p_myself->water.prepareRendering(target_framebuffer, i);
        p_myself->water.render();
        p_myself->water.finalizeRendering();
    }

    for (int i = 0; i < 5; ++i)
        p_myself->water.step(0.002f);
}


void ShallowWaterScene::onReflectionMapUpdate(Framebuffer& target_framebuffer)
{
...
void ShallowWaterScene::onKeyPress(GLFWwindow* p_window, int key, int scancode, int actions, int mods)
{
...
void ShallowWaterScene::onMouseMove(GLFWwindow* p_window, double xpos, double ypos)
{
...
void ShallowWaterScene::onScreenSizeChange(Screen& screen, int width, int height)
{
    //Update screen size
    ...
    p_myself->water.setScreenSize(uvec2{ static_cast<uint32_t>(width), static_cast<uint32_t>(height) });
}


void ShallowWaterScene::onMouseClick(GLFWwindow* p_glfw_window, int button, int action, int mods)
{
    ...
    if (!p_myself->suppress_mouse_user_input)
    {
        ...
        if (action == GLFW_PRESS && button == GLFW_MOUSE_BUTTON_2)
        {
            ...
            //Get the current state from the water simulation system
            p_myself->water.retrieveCurrentSystemState(p_myself->current_water_levels, p_myself->current_hu, p_myself->current_hv);


            if (selected_object_id == p_myself->tess_terrain.getId() || selected_object_id == p_myself->water.getId())
            {
                ...
                for (uint32_t i = 0; i < topography_y_res; ++i)
                {
                    for (uint32_t j = 0; j < topography_x_res; ++j)
                    {
                        ...
                        if (dist < 0.02f + p_myself->modification_strength*(0.05f - 0.02f))
                        {
                            ...
                            case UserInteractionMode::ADD_WATER:
                                p_myself->current_water_levels[i*topography_x_res + j] +=
                                    std::exp(-dist*dist * (5 + p_myself->modification_strength*(10 - 5)))*(0.1f + p_myself->modification_strength*(0.2f - 0.1f)) * p_myself->topography_max_height;
                                break;

                            case UserInteractionMode::REMOVE_WATER:
                                p_myself->current_water_levels[i*topography_x_res + j] -=
                                    std::exp(-dist*dist * (5 + p_myself->modification_strength*(10 - 5)))*(0.1f + p_myself->modification_strength*(0.2f - 0.1f)) * p_myself->topography_max_height;
                                break;

                            case UserInteractionMode::STIR_WATER:
                            {
                                float hu, hv;
                                ...
                                else
                                {
                                    ...
                                    hu = (10.0f + p_myself->modification_strength*20.0f) / fDimensionalDistance * v2Aux.x;
                                    hv = (10.0f + p_myself->modification_strength*20.0f) / fDimensionalDistance * v2Aux.y;
                                }
                                p_myself->current_hu[i*topography_x_res + j] += hu;
                                p_myself->current_hv[i*topography_x_res + j] += hv;
                ...
                if (p_myself->interaction_mode == SLIDE_TERRAIN)
                {
                    ...
                    for (uint32_t i = num_points_slide_down; i < terrain_slide_region.size(); ++i)
                    {
                        ...
                        p_myself->current_hu[k*topography_x_res + l] += hu;
                        p_myself->current_hv[k*topography_x_res + l] += hv;
                    }
                ...
                {
                    //Compute water elevations
                    for (uint32_t i = 0; i < topography_y_res; ++i)
                    {
                        for (uint32_t j = 0; j < topography_x_res; ++j)
                        {
                            p_myself->current_water_elevations[i*topography_x_res + j] =
                                p_myself->current_water_levels[i*topography_x_res + j] -
                                p_myself->p_interpolated_topography_data[(2 * i + 1)*(2 * topography_x_res + 1) + 2 * j + 1] * p_myself->topography_max_height;
                        }
                    }

                    KPWater::computeTopographyBilinearInterpolation(p_myself->p_raw_topography_data, topography_x_res, topography_y_res, p_myself->p_interpolated_topography_data);
                    KPWater::extractTopographyPrincipalComponent(p_myself->p_interpolated_topography_data, topography_x_res, topography_y_res, p_myself->p_processed_topography_data);

                    p_myself->water.updateTopography(std::vector < float > {p_myself->p_interpolated_topography_data,
                        p_myself->p_interpolated_topography_data + (2 * topography_x_res + 1)*(2 * topography_y_res + 1)});
                    ...
                    for (uint32_t i = 0; i < topography_y_res; ++i)
                    {
                        for (uint32_t j = 0; j < topography_x_res; ++j)
                        {
                            p_myself->current_water_levels[i*topography_x_res + j] = p_myself->current_water_elevations[i*topography_x_res + j] +
                                p_myself->p_interpolated_topography_data[(2 * i + 1)*(2 * topography_x_res + 1) + 2 * j + 1] * p_myself->topography_max_height;
                        }
                    }
                }
                ...
                p_myself->water.updateSystemState(p_myself->current_water_levels, p_myself->current_hu, p_myself->current_hv);
...
void ShallowWaterScene::onScroll(GLFWwindow* p_glfw_window, double xoffset, double yoffset)
{
    TwEventMouseWheelGLFW(static_cast<int>(yoffset));
}


void ShallowWaterScene::onCharInput(GLFWwindow* p_glfw_window, unsigned int codepoint)
{
    TwEventCharGLFW(codepoint, 1);
}


void ShallowWaterScene::onParamSet(const void* value, void* client_data)
{
...
void ShallowWaterScene::onParamGet(void* value, void* client_data)
{
    p_myself->suppress_mouse_user_input = true;
    std::string param_name{ static_cast<const char*>(client_data) };
...
fog_height_fall_off{ 0.5f }
{
    ...
    topography_max_height *= 0.8f;


    //Allocate storage for the simulator system state buffer
    current_water_levels.resize(topography_x_res*topography_y_res);
    current_water_elevations.resize(topography_x_res*topography_y_res);
    current_hu.resize(topography_x_res*topography_y_res);
    current_hv.resize(topography_x_res*topography_y_res);
    ...
    KPWater::computeTopographyBilinearInterpolation(p_raw_topography_data, topography_x_res, topography_y_res, p_interpolated_topography_data);
    KPWater::extractTopographyPrincipalComponent(p_interpolated_topography_data, topography_x_res, topography_y_res, p_processed_topography_data);
    ...
    //Configure the KP- shallow water
    p_init_water_levels = new SaintVenantSystem::Numeric[topography_x_res*topography_y_res];
    p_init_hu = new SaintVenantSystem::Numeric[topography_x_res*topography_y_res];
    p_init_hv = new SaintVenantSystem::Numeric[topography_x_res*topography_y_res];
    SaintVenantSystem::Numeric dx = 600.0f / topography_x_res, dy = 600.0f / topography_y_res;
    auto depth_func = [](float x, float e1, float e2, float L)->float
    {
        if (x <= e1) return L;
        if (x >= e2) return x;

        float aux = (2 * x - e1 - e2) / (e2 - e1);
        return 0.25f*((1 - e2 + L)*std::pow(aux, 3) + std::pow(aux, 2) + (3 * e2 - 3 * L - 1)*aux + 2 * L + 2 * e2 - 1);
    };
    for (unsigned int i = 0; i < topography_y_res; ++i)
        for (unsigned int j = 0; j < topography_x_res; ++j)
        {
            p_init_water_levels[i*topography_x_res + j] = depth_func(p_processed_topography_data[i*topography_x_res + j], 0.1f, 0.5f, 0.2f);
            p_init_hu[i*topography_x_res + j] = 0.0f;
            p_init_hv[i*topography_x_res + j] = 0.0f;
        }

    float water_scale_x = 600 / (dx*topography_x_res), water_scale_z = 600 / (dy*topography_y_res);
    water.scale(water_scale_x, topography_max_height, water_scale_z);
    water.setScreenSize(screen_size.first, screen_size.second);
    water.setLODFactor(15.0f);
    water.setLocation(vec3{ 0, 0, 0 });
    water.setColorExtinctionBoundary(5 * topography_max_height);
    water.setBloomMinimalThreshold(0.8f);
    water.setBloomMaximalThreshold(5.0f);
    water.useBloom(true);
    water.setBloomIntensity(0.5f);
    water.setSpecularExponent(100.0f);
    water.applyLightingConditions(lighting);
    water.setSpecularColor(vec3(1.0f));
    water.applyEnvironmentMap(reflection_map);
    water.setWindVelocity(vec2{ 10.0f, 10.0f });
    water.setDeepWaterWavesChoppiness(0.0f);
    water.setMaximalRippleWaveAmplitude(20.0f);
    water.setRippleSimulationTimeStep(vec2{ 1e-2f, 5e-2f });
    water.setCausticsPower(8);
    water.setCausticsAmplificationFactor(0.6f);
    water.setCausticsSampleArea(1e-3f);
    water.setMaximalDeepWaveAmplitude(0.0f);
    water.setDeepWaterRippleMapTilingFactor(2U, 30U);
    water.setMaximalWaveHeightAsElevationFraction(0.5f);
    water.initialize(p_init_water_levels, p_init_hu, p_init_hv, topography_x_res, topography_y_res,
        dx, dy, std::max(std::pow(dx*water_scale_x, 4), std::pow(dy*water_scale_z, 4)), p_interpolated_topography_data, 9.8f, 1.5f, KPWater::ODESolver::Euler);

    KPWater water1 = water;
...
ShallowWaterScene::~ShallowWaterScene()
{
    ...
    if (p_init_water_levels) delete[] p_init_water_levels;
    if (p_init_hu) delete[] p_init_hu;
    if (p_init_hv) delete[] p_init_hv;

    TwTerminate();
}


void ShallowWaterScene::init_ss_filters()
{
    auto screen_size = p_screen->getScreenSize();
...
void ShallowWaterScene::init_rendering_composer()
{
    auto screen_size = p_screen->getScreenSize();
...
void ShallowWaterScene::init_reflection_buffer(uint32_t reflection_buffer_resolution)
{
    auto screen_size = p_screen->getScreenSize();
...
void ShallowWaterScene::init_toolbar()
{
