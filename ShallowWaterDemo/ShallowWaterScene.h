...
#ifndef SHALLOW_WATER_SCENE
#define SHALLOW_WATER_SCENE
...
private:
    ...
    KPWater water;	//water based on Kurganov-Petrova solver
    ...
    SaintVenantSystem::Numeric* p_init_water_levels;	//array containing initial water levels
    SaintVenantSystem::Numeric *p_init_hu, *p_init_hv;	//arrays containing initial velocity fields for the SWE simulation
    std::vector<float> current_water_levels;	//receives the most recent water levels from the simulator when necessary
    std::vector<float> current_water_elevations;	//receives the most recent water elevations from the simulator when necessary
    std::vector<float> current_hu, current_hv;	//receive the most recent velocity fields from the simulator when necessary
    ...
    ShallowWaterScene(ShallowWaterScene&& other) = delete;	//no moves allowed
    ShallowWaterScene& operator=(const ShallowWaterScene& other) = delete;	//no copy assignments are possible
    ShallowWaterScene& operator=(ShallowWaterScene&& other) = delete;	//no move assignments are possible
    ~ShallowWaterScene();	//destructor
