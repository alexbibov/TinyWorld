...
layout(rg32f) uniform restrict imageBuffer ibGradientGrid;	//buffer texture receiving the gradient field required for creation of the Perlin noise
...
void main()
{
	...
	if(any(greaterThanEqual(gl_GlobalInvocationID.xy, vec2(uv2GradientGridSize)))) return;

	uint uAux = gl_GlobalInvocationID.y*gl_NumWorkGroups.x*gl_WorkGroupSize.x + gl_GlobalInvocationID.x + 1;
	uAux ^= uAux << 13;
	...
	{
		float fUpdate = fGradientUpdateRate*dot(BoxMuller(v2Aux), vec2(0.5f, 0.5f));
		vec2 v2OldGradient = imageLoad(ibGradientGrid, int(uOffset + gl_GlobalInvocationID.y*uv2GradientGridSize.x + gl_GlobalInvocationID.x)).rg;
		float fOldGradientAngle = acos(v2OldGradient.x*msgn(v2OldGradient.y)) + float(v2OldGradient.y<0)*pi;
		float fNewGradientAngle = fOldGradientAngle + fUpdate;
		imageStore(ibGradientGrid, int(uOffset + gl_GlobalInvocationID.y*uv2GradientGridSize.x + gl_GlobalInvocationID.x), vec4(cos(fNewGradientAngle), sin(fNewGradientAngle), 0, 0));
	}
	else
	{
		float fAngle = 2*pi*dot(v2Aux, vec2(0.5f, 0.5f));
		imageStore(ibGradientGrid, int(uOffset + gl_GlobalInvocationID.y*uv2GradientGridSize.x + gl_GlobalInvocationID.x), vec4(cos(fAngle), sin(fAngle), 0, 0));
	}
}