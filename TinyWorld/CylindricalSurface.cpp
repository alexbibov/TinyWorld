...
void CylindricalSurface::process_surface_data(std::vector<vec2>& surface_map_data, uint32_t num_points_per_slice, uint32_t num_slices)
{
	...
	for (unsigned int i = 0; i <= num_angular_base_nodes; ++i)
	{
		...
		for (unsigned int j = 0; j < num_points_per_slice; ++j)
		{
			...
			unsigned int mdist_idx = std::min_element(dist, dist + 3, [](float elem1, float elem2) -> bool {return std::abs(elem1) < std::abs(elem2); }) - &dist[0];
			...
			mdist_idx = std::min_element(dist, dist + 3, [](float elem1, float elem2) -> bool {return std::abs(elem1) < std::abs(elem2); }) - &dist[0];
