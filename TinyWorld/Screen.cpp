...
Screen::Screen(MULTISAMPLING_MODE ms_pixel_mode /* = MULTISAMPLING_MODE::MULTISAMPLING_NONE */,
	...
	monitor{ NULL }, monitor_index{ -1 }, monitor_name(""),
	p_glfwWindow{ NULL }, 
	vsync_interval{ 0 },
	...
	screen_close_callback{ [](Screen&)->void{} }
{
	...
	for (int i = 0; i < count; ++i)
		video_modes.push_back(ScreenVideoMode{ video_mode_list[i].width, video_mode_list[i].height, 
			video_mode_list[i].redBits, video_mode_list[i].greenBits, video_mode_list[i].blueBits, 
			video_mode_list[i].refreshRate });
}
...
Screen::Screen(uint32_t monitor_index, 
	...
	bool debug_mode /*= false */, bool stereo_rendering /* = false */) : 
	AbstractRenderingDevice{ "Screen" }, GLEW_initialized{ false },
	monitor_index{ monitor_index }, multisampling_mode{ ms_pixel_mode }, debug{ debug_mode },
	stereo{ stereo_rendering }, monitor{ NULL }, monitor_name(""),
	p_glfwWindow{ NULL }, 
	vsync_interval{ 0 },
	renderer{ [](Screen&)->void{} },
	screen_changesize_callback{ [](Screen&, int, int)->void{} },
	...
	screen_close_callback{ [](Screen&)->void{} }
{
	...
	for (int i = 0; i < count; ++i)
		video_modes.push_back(ScreenVideoMode{ video_mode_list[i].width, video_mode_list[i].height,
		video_mode_list[i].redBits, video_mode_list[i].greenBits, video_mode_list[i].blueBits, video_mode_list[i].refreshRate });
}
	...
	screen_close_callback{ [](Screen&)->void{} }
{
	...
	ScreenVideoMode windowed_regime_video_mode = { width, height,
		current_video_mode->redBits, current_video_mode->greenBits, current_video_mode->blueBits, 
		current_video_mode->refreshRate };
...
ScreenVideoMode Screen::getCurrentVideoMode() const
{
	if (Screen::glfw_initialization_wrapper)
	{
		if (monitor != NULL)
		{
			...
			ScreenVideoMode screen_video_mode = { current_video_mode->width, current_video_mode->height,
				current_video_mode->redBits, current_video_mode->greenBits, current_video_mode->blueBits,
				current_video_mode->refreshRate };
			...
			if ((name = glfwGetMonitorName(monitors[i])) != NULL && monitor_name.compare(name) == 0)
			{
				...
				if ((monitor_video_modes = glfwGetVideoModes(monitors[i], &video_modes_count)) != NULL)
					for (int j = 0; j < video_modes_count; ++j)
						rv.push_back(ScreenVideoMode{ monitor_video_modes[j].width, monitor_video_modes[j].height,
						monitor_video_modes[j].redBits, monitor_video_modes[j].greenBits, monitor_video_modes[j].blueBits,
						monitor_video_modes[j].refreshRate });
				break;
			}
		}
	}
	...
	if (Screen::glfw_initialization_wrapper && (monitors = glfwGetMonitors(&count)) != NULL)
	{
		...
		if ((monitor_video_modes = glfwGetVideoModes(monitors[monitor_index], &video_modes_count)) != NULL)
			for (int i = 0; i < video_modes_count; ++i)
				rv.push_back(ScreenVideoMode{ monitor_video_modes[i].width, monitor_video_modes[i].height,
				monitor_video_modes[i].redBits, monitor_video_modes[i].greenBits, monitor_video_modes[i].blueBits,
				monitor_video_modes[i].refreshRate });
	}
